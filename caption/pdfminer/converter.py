...
class HTMLConverter(PDFConverter):
    ...
    def put_text(self, text, fontname, fontsize):
        ...
        self.write_text(text)
        f = open("font.txt", "a")
        f.write("size = %d " % (fontsize * self.scale * self.fontscale) + enc(fontname) + "\n")
        f.close()
        # f = open("r.txt", "a", codec='utf-8')
        # ss = u'Capit\xe1n'
        # ss8 = ss.encode('utf8')
        # f = open("r.txt", "a"), ss8
        # # f = open('r.txt').write().encode('utf8')
        # f.write('font="%s"\nsize="%.3f"\ntext%s\n' %
        #                          (enc(fontname), fontsize * self.scale * self.fontscale, text))
